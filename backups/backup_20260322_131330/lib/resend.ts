 ...
 * AJUSTADO: Agora monta URL correta para modo revisão no dashboard
 */
export async function sendAgreementRevisionRequested(
  ...
  // NOVOS PARÂMETROS para montar URL correta (retrocompatível - agreementLink como fallback)
  agreementId: string,
  protocol: string,
  hash: string,
  // Parâmetro opcional para compatibilidade com chamadas antigas
  agreementLink?: string
) {
  try {
    // MONTAR URL CORRETA para modo revisão no dashboard do freela
    // NÃO é o link público /p/[protocol], é o formulário de formalização
