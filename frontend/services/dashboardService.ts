...
export async function getFreelancerDashboardDataV2(
  ...
  console.log('[DEBUG] Chamando Cloud Function getFreelancerDashboard com freelancerId:', freelancerId);
const callable = httpsCallable(functions, 'getFreelancerDashboard');
  const response: any = await callable({ freelancerId });
  const data = response.data;
console.log('[DASHBOARD SERVICE] Resposta da Cloud Function:', {
  totalAgreements: data.agreements?.length,
  firstAgreement: data.agreements?.[0]?.title,
  stats: data.stats
});
