...
export async function getAgreementById(id: string): Promise<Agreement | null> {
  try {
    ...
    return {
      ...
      // ⭐ NOVOS CAMPOS - Limite de 1 revisão
      revisionCount: data.revisionCount ?? 0,
      revisionLimitReached: data.revisionLimitReached || false,
      finalVersion: data.finalVersion || false,
...
export async function resendAgreementAfterRevision(
  ...
  const eventData = {
    ...
    actorType: 'freelancer',
    actorName: original.freelancerName || 'Freelancer',
    title: 'Acordo reenviado',
    description: 'Freelancer ajustou e reenviou o acordo.',
    createdAt: timestamp
  };
...
export async function getAgreementStats(agreementId: string): Promise<{
  ...
  const q = query(chargesRef, where('agreementId', '==', agreementId));
  const snapshot = await getDocs(q);
