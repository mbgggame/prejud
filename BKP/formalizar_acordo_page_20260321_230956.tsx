...
function formatDateToInput(value: unknown): string {
  if (!value) return "";
  ...
  if (value instanceof Date) {
    ...
    } else if ("_seconds" in value && typeof (value as any)._seconds === "number") {
...
export default function FormalizarAcordoPage(): React.JSX.Element {
  ...
  const originalAgreementId = searchParams.get("originalAgreementId");
  const originalProtocol = searchParams.get("originalProtocol");
  ...
  useEffect(() => {
    const loadAgreementForRevision = async () => {
      if (!isRevisionMode || !agreementId) return;
      ...
      try {
        ...
        if (!agreement) {
          alert("Não foi possível localizar o acordo original para revisão.");
          return;
        }
        ...
        setFormData({
          ...
          value:
            typeof agreement.value === "number"
              ? formatCurrencyFromNumber(agreement.value)
              : "",
          deadline: formatDateToInput(agreement.deadline),
        });
      } catch (error) {
        console.error("Erro ao carregar acordo para revisão:", error);
        alert("Erro ao carregar os dados do acordo original.");
      } finally {
        setLoadingOriginalAgreement(false);
      }
    };

    loadAgreementForRevision();
  }, [agreementId, isRevisionMode]);
  ...
  const validateForm = (): boolean => {
    ...
    } else if (isNaN(parseFloat(formData.value.replace(/[R$\s]/g, "").replace(/\./g, "").replace(",", ".")))) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const numericValue = parseFloat(
        formData.value.replace(/[^\d,]/g, "").replace(/\./g, "").replace(",", ".")
      );
      ...
      const agreementData = {
        ...
        terms: isRevisionMode
          ? "Versão revisada do acordo original"
          : "Termos padrão do acordo",
        status: "pending_client_confirmation" as AgreementStatus,
      };

      const newAgreement = await createAgreement(agreementData);

      if (isRevisionMode && originalAgreement) {
        const originalId = originalAgreementId || originalAgreement.id;

        await updateDoc(doc(db, "agreements", newAgreement.id), {
          originalAgreementId: originalId,
          revisionFromAgreementId: originalId,
          parentAgreementId: originalId,
          originalProtocol: originalProtocol || originalAgreement.protocol || "",
          originalHash: originalHash || originalAgreement.hash || "",
          revisionMode: true,
          revisionCreatedAt: serverTimestamp(),
          updatedAt: serverTimestamp(),
        });

        await updateDoc(doc(db, "agreements", originalId), {
          revisedAgreementId: newAgreement.id,
          hasRevisedVersion: true,
          revisionStatus: "resent",
          updatedAt: serverTimestamp(),
        });
      }
      ...
      <main className="mx-auto max-w-7xl px-4 pb-8 pt-28 sm:px-6 sm:pt-32 lg:px-8 lg:pt-36">
        ...
        <section className="rounded-[28px] border border-slate-200 bg-white p-6 shadow-sm dark:border-white/10 dark:bg-[#0F172A] dark:shadow-none">
          <div className="mb-4 inline-flex items-center gap-2 rounded-full border border-blue-200 bg-blue-50 px-3 py-1.5 text-xs font-semibold text-blue-700 dark:border-blue-500/20 dark:bg-blue-500/10 dark:text-blue-300">
            ...
            {isRevisionMode ? "Revisão de acordo" : "Novo acordo"}
          </div>

          <h1 className="text-2xl font-bold tracking-tight text-slate-900 dark:text-white sm:text-3xl lg:text-4xl">
            {isRevisionMode ? "Reenviar acordo revisado" : "Formalizar novo acordo"}
          </h1>

          <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600 dark:text-slate-300">
            {isRevisionMode
              ? "Revise os dados do acordo original, faça os ajustes necessários e reenviе uma nova versão ao cliente."
              : "Crie um acordo digital com trilha de auditoria, assinatura eletrônica e garantia de compliance jurídico."}
          </p>

          {isRevisionMode && (
            <div className="mt-4 rounded-2xl border border-blue-200 bg-blue-50 px-4 py-3 text-sm text-blue-800 dark:border-blue-500/20 dark:bg-blue-500/10 dark:text-blue-300">
              <p className="font-semibold">Modo revisão ativo</p>
              <p className="mt-1">
                {loadingOriginalAgreement
                  ? "Carregando dados do acordo original..."
                  : `Você está revisando o acordo ${originalAgreement?.title ? `"${originalAgreement.title}"` : "selecionado"}.`}
              </p>
              {originalProtocol && (
                <p className="mt-1 text-xs opacity-80">Protocolo original: {originalProtocol}</p>
              )}
            </div>
          )}
        </section>
        ...
        <section className="mt-6 rounded-[28px] border border-slate-200 bg-white shadow-sm dark:border-white/10 dark:bg-[#0F172A] dark:shadow-none">
          <div className="border-b border-slate-100 px-5 py-5 sm:px-6 sm:py-6 dark:border-white/10">
            ...
            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">
              {isRevisionMode
                ? "Atualize as informações abaixo antes de reenviar a nova versão"
          ...
          <form onSubmit={handleSubmit} className="p-5 sm:p-6">
            ...
            <div className="mt-8 flex flex-col-reverse gap-3 border-t border-slate-100 pt-6 dark:border-white/10 sm:flex-row sm:justify-end sm:gap-4">
              ...
              <button
                type="submit"
                disabled={isSubmitting || loadingOriginalAgreement}
              ...
              >
                {isSubmitting ? (
                  <>
                    ...
                    {isRevisionMode ? "Reenviando acordo..." : "Criando acordo..."}
                  </>
                ) : (
                  <>
                    <FileSignature className="h-4 w-4" />
                    {isRevisionMode ? "Reenviar acordo revisado" : "Criar acordo"}
                  </>
                )}
              </button>
            </div>
